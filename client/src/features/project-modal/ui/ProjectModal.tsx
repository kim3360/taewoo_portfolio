    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          {/* Modal */}
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} transition={{ type: "spring", damping: 20, stiffness: 300 }} className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={(e) => e.stopPropagation()}>
            <div className="bg-card border border-border rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
              ...
              <div className=" static top-0 flex items-center justify-between p-6 border-b border-border bg-card/95 backdrop-blur-sm z-10">
                <div className="flex items-center gap-4">
                  {project.logo && <img src={project.logo} alt={`${project.title} logo`} className="w-12 h-12 rounded-lg object-contain" />}
              ...
              <div className="overflow-y-auto flex-1">
                <div className="p-6 space-y-6">
                  ...
                  <div className="grid grid-cols-3 gap-4 pt-4 border-t border-border">
                    {project.members && (
                      <div>
                        <h4 className="text-sm font-semibold mb-2 text-muted-foreground">참여인원</h4>
                    ...
                    {project.period && (
                      <div>
                        <h4 className="text-sm font-semibold mb-2 text-muted-foreground">기간</h4>
                        <div className="flex items-center gap-2">
                    ...
                    {project.pdf && (
                      <div>
                        <h4 className="text-sm font-semibold mb-2 text-muted-foreground">PDF 파일</h4>
                        <div className="flex items-center gap-2">
                          <FileDown className="w-4 h-4 text-primary" />
                          <a href={project.pdf} download className="text-sm text-primary hover:underline">
                            PDF 다운로드
                          </a>
                        </div>
                      </div>
                  ...
                  <div className="flex gap-3 pt-4">
                    {project.liveDemo ? (
                      ...
                      <div className="flex-1">
                        <Button disabled className="w-full bg-primary/50 hover:bg-primary/50 cursor-not-allowed opacity-60">
                          <ExternalLink className="w-4 h-4 mr-2" />
                          라이브 데모
                        </Button>
                      </div>
